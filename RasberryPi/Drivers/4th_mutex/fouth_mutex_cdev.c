...
static DEFINE_MUTEX(test_cdev_mutex);  /// A macro that is used to declare a new mutex that is visible in this file
                                     /// results in a semaphore variable test_cdev_mutex with value 1 (unlocked)
...
{
    //if(!mutex_trylock(&test_cdev_mutex)){    /// Try to acquire the mutex (i.e., put the lock on/down)
    mutex_lock(&test_cdev_mutex);    /// Try to acquire the mutex (i.e., put the lock on/down)
                                          /// returns 1 if successful and 0 if there is contention
      printk(KERN_ALERT CHAR_DEV_NAME": Device in use by another process");
     // return -EBUSY;
    //}
