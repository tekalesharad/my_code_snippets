#include "rspi_intr_workq_tasklet.h"
...
static void tasklet_bottom_half(unsigned long val)
{

  printk(KERN_INFO "######## TASK_LET");
...
static int __init ledmodule_init(void) 
{
  ...
  tasklet_init(&tsklet, &tasklet_bottom_half, NULL );
