...
irqreturn_t pushbtn_irq_handle(int irq ,  struct pt_regs *regs)
{
  printk(KERN_INFO "######## received irq[%d]", irq);
  return IRQ_HANDLED;
}
...
static int configure_gpio()
{
  ...
  err = request_irq(irq_num,  pushbtn_irq_handle, IRQF_TRIGGER_RISING, CHAR_DEV_NAME, NULL );
  if(err < 0)
    return err;
...
static int __exit ledmodule_exit(void) 
{
  int  i =0;
  //Free GPIOs
